...
import { selectInventoryById, useImportInventoryMutation } from "@/redux/services/inventory-api"
...
export const InventoryForm: React.FC<InventoryFormProps> = ({ inventoryId }) => {

    const [ingredients, setIngredients] = useState<IngredientType[]>(useSelector(selectIngredients))
    ...
    const inventory = useSelector(state => selectInventoryById(state, inventoryId as EntityId))
    ...
    const onSubmit = async (data: InventoryFormValues) => {
        ...
        if (data) {
            await importInventory({ ingredient_id: data.ingredient_id, payload: { quantity: data.quantity } })
        } else {
                                    ...
                                    <Select
                                        ...
                                        <FormControl>
                                            <SelectTrigger className="w-[180px]">
                                                <SelectValue defaultValue={field.value} placeholder="Select an ingredient" />
                                            </SelectTrigger>
                                        </FormControl>
                                        <SelectContent>
                                            {ingredients != undefined && ingredients.map((ingredient) => (
                                                <SelectItem key={ingredient.id} value={ingredient.id.toString()}>{ingredient.name}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                    <FormMessage />
